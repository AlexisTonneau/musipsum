        ...
        if (isset($_SESSION['user']) && unserialize($_SESSION['user'])->getAccountType()==1){
            if (isset($_GET['ref'])) {
                switch ($_GET['ref']){   //TODO Modify buttons of the page @DEMARS
                    case 'newaccount':
                        require('english/views/views_admin/viewCreateUser.php');
                        break;
                    case 'handlecaptor':
                        require_once('english/views/views_admin/viewHandleCaptors.php');
                        break;
                    case 'disconnect':
                        session_destroy();
                        header('Location: ' .URL);
                    ...
                    case 'modify-drivingschool':
                        ...
                        require_once('english/views/views_admin/viewModifyDrivingSchool.php');
                        break;
                    case 'search':
                        if(!isset($_POST['modify'])) {
                            require_once('english/views/views_admin/viewUsersResearch.php');
                        }
                        else{
                ...
                require('english/views/views_admin/viewUsersAdmin.php');
            }
        }
        else{
            header('Location: '.URL);
